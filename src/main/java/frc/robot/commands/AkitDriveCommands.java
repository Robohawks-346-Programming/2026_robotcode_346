...
public class AkitDriveCommands {
        ...
        private static final double SPEED_SCALE = 0.75; // Scale down max speed (0.3 = 30% of max speed)
        private static final double ROTATION_SCALE = 0.75; // Scale down max speed (0.3 = 30% of max speed)
         ...
         * @param xSupplier        Supplier for forward/backward joystick input (-1 to
         *                         1)
         ...
         * @param useFieldRelative Supplier for whether to use field-relative mode
         *                         (default: robot-relative)
         */
        ...
        private static final double AIM_MIN_TARGET_DISTANCE_METERS = 0.15;

        /**
         * Drive with optional aim-to-point: when aimButton is pressed, robot faces
         * aimTarget,
         * right stick is disabled, and left stick still controls translation
         * (field-relative).
         */
        public static Command joystickDriveWithAim(
                ...
                final Rotation2d[] heldAimHeading = { new Rotation2d() };
                ...
                return Commands.run(
                                () -> {
                                        ...
                                        if (aiming) {
                                                // Aim mode: face the target point, ignore right stick
                                                Pose2d pose = drive.getPose();
                                                Rotation2d robotRotation = drive.getRotation();
                                                Translation2d targetVector = aimTarget.minus(pose.getTranslation());
                                                Rotation2d targetHeading = targetVector
                                                                .getNorm() <= AIM_MIN_TARGET_DISTANCE_METERS
                                                                                ? heldAimHeading[0]
                                                                                : targetVector.getAngle().plus(new Rotation2d(Math.PI));
                                                double measurementRad = robotRotation.getRadians();
                                                double setpointRad = targetHeading.getRadians()- Math.PI;
                                                double errorRad = (targetHeading.minus(robotRotation).getRadians() - Math.PI);
                                                double maxAimOmegaRadPerSec = drive.getMaxAngularSpeedRadPerSec()
                                                                * AIM_MAX_SPEED_SCALE;
                                                double absErrorRad = Math.abs(errorRad);

                                                // On initial LT press, reset controller state to avoid a one-cycle
                                                // jump.
                                                if (!wasAiming[0]) {
                                                        heldAimHeading[0] = robotRotation;
                                                        aimController.reset(measurementRad);
                                                        aimOmegaLimiter.reset(0.0);
                                                        aimLocked[0] = false;
                                                }
                                                if (targetVector.getNorm() > AIM_MIN_TARGET_DISTANCE_METERS) {
                                                        heldAimHeading[0] = targetHeading;
                                                }
                                                ...
                                                if (aimLocked[0] && Math.abs(errorRad) <= AIM_UNLOCK_TOLERANCE_RAD) {
                                                        ...
                                                        double desiredOmegaRadPerSec = MathUtil.clamp(
                                                                        aimController.calculate(measurementRad,
                                                                                        setpointRad),
                                                                        -maxAimOmegaRadPerSec,
                                                                        maxAimOmegaRadPerSec);
                                                        omegaRadPerSec = aimOmegaLimiter
                                                                        .calculate(-desiredOmegaRadPerSec);

                                                        // if (Math.abs(errorRad) <= AIM_ANGLE_TOLERANCE_RAD &&
                                                        // aimController.atSetpoint()) {
                                                        if (Math.abs(errorRad) <= AIM_ANGLE_TOLERANCE_RAD) {
                                                                aimLocked[0] = true;
                                                                omegaRadPerSec = 0.0;
                                                                aimController.reset(measurementRad);
                                                        ...
                                                        Logger.recordOutput("errorRad", Math.toDegrees(absErrorRad));
                                                        Logger.recordOutput("atSetpoint", aimController.atSetpoint());
                                                        Logger.recordOutput("measurementrad", measurementRad);
                                                        Logger.recordOutput("setpointrad", setpointRad);
                                                }

                                                // While large heading correction is in progress, slightly reduce
                                                // translation
                                                ...
                                                translationScale = 1.0
                                                                - (1.0 - AIM_MIN_TRANSLATION_SCALE) * errorFraction;
                                        } else {
                                                aimLocked[0] = false;
