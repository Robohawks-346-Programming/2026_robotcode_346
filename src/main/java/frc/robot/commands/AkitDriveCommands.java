        ...
        return Commands.run(
                () -> {
                    ...
                    if (aiming) {
                        ...
                        Rotation2d targetHeading = targetVector.getNorm() <= AIM_MIN_TARGET_DISTANCE_METERS
                                ? heldAimHeading[0]
                                : targetVector.getAngle();
                        double measurementRad = robotRotation.getRadians();
                        double setpointRad = targetHeading.getRadians();
                        double errorRad = targetHeading.minus(robotRotation).getRadians();
